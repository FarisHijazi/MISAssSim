...
representationFormatter = {
    ...
    'fp': lambda val: re.sub('\s', '', format(val, str(addressBits)+'b')), #TODO: format to floating point
    'hex': lambda val: re.sub('\s', '', format(val, str(addressBytes//2)+'x')),
    'bin': lambda val: re.sub('\s', '', format(val, str(addressBytes)+'b')),
    'dec': lambda val: re.sub('\s', '', format(val, str(addressBytes)+'d')),
    'ascii': lambda val: re.sub('\s', '', format(val, str(addressBytes//2)+'x')), #TODO: this is bs, just a place holder
}
